...
class GithubAPI {
  ...
  get(url, done, query) {
    request.get(url).auth(this.credential.username, this.credential.token).query(query).set('Accept', 'application/vnd.github.v3+json').end(done);
  }
  ...
  put(url, data, done) {
    request.put(url).auth(this.credential.username, this.credential.token).send(data).set('Accept', 'application/vnd.github.v3+json').end(done);
  }
